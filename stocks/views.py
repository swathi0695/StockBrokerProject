    ...
    def get(self, request: Request) -> Response:
        ...
        for stock_data in serialized_stocks:
            ...
            if Market.objects.filter(stock=stock).exists():
                latest_price = (
                    Market.objects.filter(stock=stock)
                    .order_by("-timestamp")
                    .first()
                    .price
                )
                ...
                stock_data["latest_price"] = Stock.objects.get(
                    pk=stock_data["name"]
                ).price
...
class StockPriceView(generics.RetrieveAPIView):
    ...
    def get(self, *args: Any, **kwargs: Any) -> Response:
        ...
        stock = Stock.objects.get(pk=stock_name)
        latest_price = (
            Market.objects.filter(stock=stock).order_by("-timestamp").first().price
        )
...
class BuySellStockView(generics.CreateAPIView):
    ...
    def create(self, request: Request, *args, **kwargs) -> Response:
        ...
        try:
            investor_current_shares = Investor.objects.get(
                name=request.user, stock=stock
            ).purchased_shares
        except:
            Investor.objects.create(
                name=self.request.user, stock=stock, purchased_shares=stock_quantity
            )
        ...
        if stock_type == "Buy":
            Stock.objects.filter(pk=stock_name).update(
                quantity=(available_units - stock_quantity)
            )

            Investor.objects.filter(name=request.user, stock=stock).update(
                purchased_shares=investor_current_shares + stock_quantity
            )
            ...
            return Response(
                {"detail": "Stock bought successfully."}, status=status.HTTP_201_CREATED
            )
